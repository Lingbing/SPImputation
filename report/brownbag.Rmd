...
# `CUTOFF` method (take monthly data for example)
For each missing value, we try to complete it by following steps:
* create a reference file with a few station that have a high correlation ($\rho$) with the candidate station. We have developed a cross-validation and also a simulation procedure to choose an optimal $\rho$. 
